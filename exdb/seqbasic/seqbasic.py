...
#   
    ...
    while i < N_QUOTES:
        quote.timestamp.append( [i+j+1 for j in range(BUF_SIZE)]) 
        quote.open.append(op)
        quote.close.append(cl)
        quote.high.append(hg)
        quote.low.append(lw)
        quote.volume.append( [i+j for j in range(BUF_SIZE)] )
        i += BUF_SIZE
    con.commit()
...
#   
    ...
    while i < N_QUOTES:
        quote.timestamp.append(array.array('I', [i+j+1 for j in range(BUF_SIZE)]))
        quote.open.append(op)
        quote.close.append(cl)
        quote.high.append(hg)
        quote.low.append(lw)
        quote.volume.append( array.array('I', [i+j for j in range(BUF_SIZE)] ) )
        i += BUF_SIZE
...
def aggregate_quotes(conn):
    ...
    tsum = next(quote.timestamp.agg_sum())
    osum = next(quote.open.agg_sum())
    clsum = next(quote.close.agg_sum())
    hsum = next(quote.high.agg_sum())
    lsum = next(quote.low.agg_sum())
    vsum = next(quote.volume.agg_sum())
...
def calc(conn):
    ...
    thninlen = next(clthin.agg_count())
