...
def createRecord(conn, i):
    ...
    rec.arr_i1  = [i + j for j in range(ARRAY_LEN)]
    rec.arr_i4  = [i + j for j in range(ARRAY_LEN)]
    rec.arr_str = ["arr_string %s %s" % (i,j) for j in range(ARRAY_LEN)]

    rec.vec_i4     = [i + j for j in range(i)] 
    rec.vec_i1     = [i + j for j in range(i)]
    rec.vec_str    = ["arr_string %s %s" % (i,j) for j in range(i)]
    rec.vec_nested = []
