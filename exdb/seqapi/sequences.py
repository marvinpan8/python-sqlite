...
def random_integer(mod):
    global seed
    seed = (seed * 3141592621 + 2718281829) % 1000000007
    log_rand()
    return seed % mod
...
def create_database():
    """
    Load eXtremeDB runtime, load dictionary, create database and open it
    :return: Pair of (database object, connection)
    """
    exdb.init_runtime(False, 'mursiw', shm=False, debug=is_debug)
...
def do_filter(con):
    ...
    for quote in cursor:
        ...
        dayit = quote.day.search(DMY(1,MONTH(last),YEAR(last)),
                                 exdb.SeqIteratorBoundary.MCO_SEQ_BOUNDARY_INCLUSIVE, last,
...
def do_VWAP(con):
    ...
    for quote in cursor:
        ...
        #Execute query
        print("%s: %f" % (quote.symbol, next(div_it)))
    cursor.close()
    con.commit()
...
def main():
    ...
    do_filter(con)
    do_correlation(con)
    do_grand_aggregate(con)
    do_window_aggregate(con)
    do_grid_aggregate(con)
    do_group_by_aggregate(con)
    do_hash_aggregate(con)
    do_sort(con)
    do_top(con)
    do_VWAP(con)
